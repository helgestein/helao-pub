...
def moveDown(dz: float,steps: float,maxForce: float, threshold:float=22.6):
    ...
    for i in range(steps):
        url_sens = "http://{}:{}".format(config['servers']['sensingServer']['host'], config['servers']['sensingServer']['port'])
        res = requests.get("{}/forceAction/read".format(url_sens), params=None).json()
