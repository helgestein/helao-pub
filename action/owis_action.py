...
def calibration(x:str,m:str,y:str,d:float,minh:float=2,maxh:float=8,dh:float=.1,f:float=5,t:float=10,key:str='raman'):
    ...
    xd = {config['owis']['roles'].index('x'):xp[0],config['owis']['roles'].index('y'):xp[1]}
    md = {config['owis']['roles'].index('x'):mp[0],config['owis']['roles'].index('y'):mp[1]}
    yd = {config['owis']['roles'].index('x'):yp[0],config['owis']['roles'].index('y'):yp[1]}
    res = []
    for i,j in xd.items():
