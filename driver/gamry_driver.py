    ...
    async def measurement_setup(self, AcqFreq, mode: Gamry_modes = None, *argv):
        ...
        if self.pstat:
            ...
            if mode == Gamry_modes.CA:
                ...
                self.FIFO_column_headings = [
                    't_s', 'Ewe_V', 'Vu', 'I_A', 'Q', 'Vsig', 'Ach_V', 'IERange', 'Overload_HEX', 'StopTest']
                ...
                Dtaqtype = None
                self.FIFO_column_headings = ['t_s', 'Ewe_V', 'Vu', 'I_A', 'Vsig',
                                             'Ach_V', 'IERange', 'Overload_HEX', 'StopTest', 'Cycle', 'unknown1']
                self.pstat.SetCtrlMode(self.GamryCOM.PstatMode)
            elif mode == Gamry_modes.LSV:
                Dtaqmode = "GamryCOM.GamryDtaqCpiv"
                Dtaqtype = None
                self.FIFO_column_headings = [
                    't_s', 'Ewe_V', 'Vu', 'I_A', 'Vsig', 'Ach_V', 'IERange', 'Overload_HEX', 'StopTest', 'unknown1']
                self.pstat.SetCtrlMode(self.GamryCOM.PstatMode)
                ...
                self.FIFO_column_headings = [
                    'I_A', 'Ewe_V', 'Zreal', 'Zimag', 'Zsig', 'Zphz', 'Zfreq', 'Zmod']
                self.pstat.SetCtrlMode(self.GamryCOM.PstatMode)
            elif mode == Gamry_modes.OCV:
                Dtaqmode = "GamryCOM.GamryDtaqOcv"
                Dtaqtype = None
                self.FIFO_column_headings = ['t_s', 'Ewe_V', 'Vm', 'Vsig', 'Ach_V',
                                             'Overload_HEX', 'StopTest', 'unknown1', 'unknown2', 'unknown3']
                self.pstat.SetCtrlMode(self.GamryCOM.PstatMode)
            else:
    ...
    async def measure(self):
        ...
        if self.pstat:
            ...
            if self.IO_TTLsend >= 0:
                ...
                print(self.pstat.SetDigitalOut(
                    self.IO_TTLsend, self.IO_TTLsend))  # bitmask on
#                print(self.pstat.SetDigitalOut(0,self.IO_TTLsend)) # bitmask off
            ...
            self.FIFO_gamryheader.replace(
                f"%epoch_ns=FIXME", f"%epoch_ns={realtime}")
            ...
            # while sink_status == "measuring" and self.IO_do_meas:
                # while sink_status == "measuring":
                # need some await points
                # await asyncio.sleep(0.001)
                # client.PumpEvents(0.001)
                
            while counter < len(self.dtaqsink.acquired_points) and self.IO_do_meas and sink_status == "measuring":
                # need some await points
                await asyncio.sleep(0.001)
                client.PumpEvents(0.001)
    ...
    async def technique_wrapper(self, act, measmode, sigfunc, sigfunc_params, samplerate, eta=0.0, setupargs=[]):
        ...
        if retval["potentiostat_connection"] == "connected":
            if self.pstat and not self.IO_do_meas:
                ...
                await self.measurement_setup(1.0/samplerate, self.IO_meas_mode, *setupargs)
    ...
    async def technique_LSV(self,
        ...
        sigfunc_params = [self.pstat, Vinit, Vfinal,
                          ScanRate, SampleRate, self.GamryCOM.PstatMode]
        ...
        activeDict = await self.technique_wrapper(A, measmode, sigfunc, sigfunc_params, SampleRate, eta)
        return activeDict
    ...
    async def technique_CA(self,
        ...
        # time expected for measurement to be completed
        eta = Tval  # +delay
        sigfunc_params = [self.pstat, Vval, Tval,
                          SampleRate, self.GamryCOM.PstatMode]
    ...
    async def technique_CP(self,
        ...
        # time expected for measurement to be completed
        eta = Tval  # +delay
        sigfunc_params = [self.pstat, Ival, Tval,
                          SampleRate, self.GamryCOM.GstatMode]
        sigfunc = "GamryCOM.GamrySignalConst"
    ...
    async def technique_EIS(self,
        ...
        self.IO_TTLwait = TTLwait
        self.IO_TTLsend = TTLsend
        activeDict = await self.technique_wrapper(A, measmode, sigfunc, sigfunc_params, SampleRate, eta, argv)
        return activeDict
    ...
    async def technique_OCV(self,
        ...
        # time expected for measurement to be completed
        eta = Tval  # +delay
        sigfunc_params = [self.pstat, 0.0, Tval,
                          SampleRate, self.GamryCOM.PstatMode]
        sigfunc = "GamryCOM.GamrySignalConst"
