...
class DataUtilSim:
    ...
    def active_learning_gaussian_simulation_parallel(self, name, num, query, data, awaitedpoints, exp_exl_ratio):
        ...
        C, I, Q = self.transformations() #already np.arrays
    ...
    def plot_variance(self, name, num, y_var, rnd_ix, quin, test_ix, train_ix):
        ...
        plt.title("Next sample will be (%.2f, %.2f) with variance (%.2f)" %
                  (quin[test_ix][rnd_ix][0], quin[test_ix][rnd_ix][1], y_var[rnd_ix]))
        # ax.autoscale(False)
        ...
        plt.ylabel("Y")
        plt.savefig(
            f"C:/Users/LaborRatte23-2/Documents/data/substrate_-999/AL/var/var_{name}_{num}.png")
        plt.clf()
    ...
    def plot_aqf(self, name, num, aqf, rnd_ix, quin, test_ix, train_ix):
        ...
        plt.title("Next sample will be (%.2f, %.2f) with acquisition of (%.2f)" % (
            quin[test_ix][rnd_ix][0], quin[test_ix][rnd_ix][1], aqf[rnd_ix]))
        # ax.autoscale(False)
        plt.axvline(quin[test_ix][rnd_ix][0], color='k')
        plt.axhline(quin[test_ix][rnd_ix][1], color='k')
        plt.scatter(quin[test_ix[rnd_ix]][0], quin[test_ix[rnd_ix]][1], color='r', label = 'Next sample')
        ...
        plt.ylabel("Y")
        plt.savefig(
            f"C:/Users/LaborRatte23-2/Documents/data/substrate_-999/AL/aqf/aqf_{name}_{num}.png")
        plt.clf()
    ...
    def plot_target(self, name, num, x, y, z_con, quin, rnd_ix, test_ix, train_ix):
        ...
        plt.title("Next sample will be (%.2f, %.2f)" %
                  (quin[test_ix[rnd_ix]][0], quin[test_ix[rnd_ix]][1]))
        plt.colorbar()
        plt.axvline(quin[test_ix[rnd_ix]][0], color='k')
        plt.axhline(quin[test_ix[rnd_ix]][1], color='k')
        plt.scatter(quin[test_ix[rnd_ix]][0], quin[test_ix[rnd_ix]][1], color='r', label = 'Next sample')
        ...
        plt.xlabel("X")
        plt.savefig(
            f"C:/Users/LaborRatte23-2/Documents/data/substrate_-999/AL/tar/tar_{name}_{num}.png")
        plt.clf()
    ...
    def plot_prediction(self, name, num, pred, rnd_ix, quin, y_query, test_ix, train_ix):
        ...
        plt.title("Next sample will be (%.2f, %.2f) with prediction (%.2f)" %
                  (quin[test_ix][rnd_ix][0], quin[test_ix][rnd_ix][1], pred[rnd_ix]))
        # ax.autoscale(False)
        plt.axvline(quin[test_ix][rnd_ix][0], color='k')
        plt.axhline(quin[test_ix][rnd_ix][1], color='k')
        plt.scatter(quin[test_ix][rnd_ix][0], quin[test_ix][rnd_ix][1], color='r', label = 'Next sample')
        plt.scatter(quin[train_ix][:,0], quin[train_ix][:,1], c=y_query[train_ix], label = 'Measured points')
        plt.xlabel("X")
        plt.ylabel("Y")
        plt.savefig(
            f"C:/Users/LaborRatte23-2/Documents/data/substrate_-999/AL/pred/pred_{name}_{num}.png")
        plt.clf()
