...
def makeApp(confPrefix, servKey):
    ...
    app = makeActServ(
        config, servKey, servKey, "PAL Autosampler Server", version=2.0, driver_class=cPAL
    )
    ...
    async def update_PAL_system_vial_table(vial: int, vol_mL: float, liquid_sample_no: int, tray: int = 2, slot: int = 1, action_params = ''):
        ...
        retc = return_class(
            ...
            data={"update": await app.driver.update_PAL_system_vial_table(tray, slot, vial, vol_mL, liquid_sample_no)},
        )
        await stat.set_idle()
        return retc
    ...
    async def write_vial_holder_table_CSV(tray: int = 2, slot: int = 1, action_params = ''):
        ...
        retc = return_class(
            measurement_type="PAL_command",
            parameters={"command": "get_vial_holder_table"},
            data={"vial_table": await app.driver.get_vial_holder_table(tray, slot, csv = True)},
        )
        await stat.set_idle()
        return retc
    ...
    #               dest_vial: int = 1, # will be filled via call to vial warehouse table
        ...
        retc = return_class(
            ...
            data=await app.driver.initcommand(cPALparams(liquid_sample_no = liquid_sample_no,
                                                method = method,
                                                tool = tool,
                                                source = source,
                                                volume_uL = volume_uL,
