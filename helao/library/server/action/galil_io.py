...
def makeApp(confPrefix, servKey):
    ...
    async def read_analog_in(
        port: Optional[int] = None, action_dict: Optional[dict] = None
    ):
        ...
        if new_port == None:
            new_port = int(port)
        await active.enqueue_data({"analog_in": app.driver.read_analog_in(new_port)})
    ...
    async def read_digital_in(
        ...
        if new_port == None:
            new_port = int(port)
        await active.enqueue_data({"digital_in": app.driver.read_digital_in(new_port)})
        finished_act = await active.finish()
        finished_dict = finished_act.as_dict()
        del finished_act
        return finished_dict
    ...
    async def read_digital_out(
        ...
        await active.enqueue_data(
            {"digital_out": app.driver.read_digital_out(new_port)}
        )
        finished_act = await active.finish()
        finished_dict = finished_act.as_dict()
        del finished_act
    ...
    async def set_digital_out_on(
        ...
        if new_port == None:
            new_port = int(port)
        await active.enqueue_data(
            {"digital_out_on": app.driver.digital_out_on(new_port)}
        )
        finished_act = await active.finish()
        finished_dict = finished_act.as_dict()
        del finished_act
    ...
    async def set_digital_out_off(
        ...
        if new_port == None:
            new_port = int(port)
        await active.enqueue_data(
            {"digital_out_off": app.driver.digital_out_off(new_port)}
        )
        finished_act = await active.finish()
        finished_dict = finished_act.as_dict()
        del finished_act
    ...
    async def set_triggered_cycles(
        ...
        await active.enqueue_data(
            {
                "digital_cycle": app.driver.set_digital_cycle(
                    trigger_port, out_port, t_cycle
                )
            }
        )
        finished_act = await active.finish()
    ...
    async def set_analog_out(
        ...
        active = await app.base.contain_action(A)
        await active.enqueue_data(
            {"analog_out": app.driver.set_analog_out(port, value)}
        )
        # await active.enqueue_data({"analog_out": app.driver.set_analog_out(handle, module, bitnum, value)})
        finished_act = await active.finish()
        finished_dict = finished_act.as_dict()
        del finished_act
        return finished_dict
    ...
    async def inf_cycles(
        ...
        await active.enqueue_data(
            {
                "analog_out": app.driver.infinite_digital_cycles(
                    time_off, time_on, port, init_time
                )
            }
        )
        finished_act = await active.finish()
    ...
    async def break_inf_cycles(action_dict: Optional[dict] = None):
        ...
        active = await app.base.contain_action(A)
        await active.enqueue_data(
            {"analog_out": app.driver.break_infinite_digital_cycles()}
        )
        finished_act = await active.finish()
        finished_dict = finished_act.as_dict()
        del finished_act
