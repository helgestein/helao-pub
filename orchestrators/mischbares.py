...
async def doMeasurement(experiment: str):
    ...
    for action_str in experiment['soe']:
        ...
        with h5py.File(tracking['path'], 'r') as session: #add metadata to experiment
            l = list(session[f"/run_{tracking['run']}/experiment_{tracking['experiment']}"].keys())
            if len(l) > 0 and 'meta' not in l: #disregard if you did not do any real actions this experiment, or if meta already added
                session.close()
                save_dict_to_hdf5({'meta':experiment['meta']},tracking['path'],path=f"/run_{tracking['run']}/experiment_{tracking['experiment']}/",mode='a')
...
def start(experiment: dict,**params):
    ...
    with h5py.File(tracking['path'], 'r') as session: #adds a new run to session to receive incoming data
        if "run_0" not in session.keys(): 
            session.close()
            save_dict_to_hdf5(dict(run_0=dict(experiment_0=None,meta=params)),tracking['path'],mode='a')
            tracking['run'] = 0
        else:
            ...
            session.close()
            save_dict_to_hdf5({run:dict(experiment_0=None,meta=params)},tracking['path'],mode='a')
            tracking['run'] = int(run[4:])
    tracking['experiment'] = 0
    save_dict_to_hdf5({'meta':None},tracking['path'],path=f'/run_{tracking["run"]}/experiment_0/',mode='a')
    return experiment
