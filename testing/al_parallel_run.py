...
query = json.dumps({'x_query': x_query.tolist(), 'y_query': y_query, 'x_grid': x_grid.tolist(), 'y_grid':y_grid.tolist(), 'z_con':z_con.tolist()})  
...
test_fnc(dict(soe=['orchestrator/start','lang:1/moveWaste_0', 'lang:1/RemoveDroplet_0', 'lang:1/moveSample_0','lang:1/moveAbs_0','lang:1/moveRel_0',
                    'lang:1/moveDown_0','measure:1/schwefelFunction_0','analysis/dummy_0'],
             params={'start': {'collectionkey' : 'al_parallel'},'moveWaste_0':{'x': 0, 'y':0, 'z':0},
                    ...
                    'schwefel_address':'experiment_0:0/schwefelFunction_0/data/data/key_y'}}, meta=dict()))
for i in range(num_sampling):
    test_fnc(dict(soe=[f'orchestrator/wait_{i}',f'orchestrator/modify_{i}',f'lang:1/moveWaste_{i+1}', f'lang:1/RemoveDroplet_{i+1}', f'lang:1/moveSample_{i+1}',
                  f'lang:1/moveAbs_{i+1}',f'lang:1/moveRel_{i+1}', f'lang:1/moveDown_{i+1}',f'measure:1/schwefelFunction_{i+1}',f'analysis/dummy_{i+1}'], 
                params={f'wait_{i}':{'addresses':f'experiment_{i}:2/activeLearningParallel_{2*i}'},
                        f'modify_{i}':{'addresses':[f'experiment_{i}:2/activeLearningParallel_{2*i}/data/data/next_x',
                                                    ...
                                                    f'experiment_{i}:2/activeLearningParallel_{2*i}/data/data/next_x',
                                                    f'experiment_{i}:2/activeLearningParallel_{2*i}/data/data/next_y'],
                        ...
                        f'dummy_{i+1}':{'x_address':f'experiment_{i+1}:0/schwefelFunction_{i+1}/data/parameters/x','y_address':f'experiment_{i+1}:0/schwefelFunction_{i+1}/data/parameters/y',
                                        'schwefel_address':f'experiment_{i+1}:0/schwefelFunction_{i+1}/data/data/key_y'}}, meta=dict()))
...
test_fnc(dict(soe=['orchestrator/wait_0','orchestrator/modify_0','lang:2/moveWaste_0', 'lang:2/RemoveDroplet_0', 'lang:2/moveSample_0',
                    'lang:2/moveAbs_0','lang:2/moveRel_0','lang:2/moveDown_0','measure:2/schwefelFunction_0','analysis/dummy_0'], 
            params={'wait_0':{'addresses':'experiment_0:2/activeLearningParallel_0'},
                   'modify_0':{'addresses':['experiment_0:2/activeLearningParallel_0/data/data/next_x_2',
                                            'experiment_0:2/activeLearningParallel_0/data/data/next_y_2',
                                            'experiment_0:2/activeLearningParallel_0/data/data/next_x_2',
                                            'experiment_0:2/activeLearningParallel_0/data/data/next_y_2'],
                   'pointers':['schwefelFunction_0/x','schwefelFunction_0/y', 'moveAbs_0/dx', 'moveAbs_0/dy']},
                    'moveWaste_0':{'x': 0, 'y':0, 'z':0},
                    'RemoveDroplet_0': {'x':0, 'y':0, 'z':0},
                    'moveSample_0': {'x':0, 'y':0, 'z':0},
                    'moveAbs_0': {'dx':'?', 'dy':'?', 'dz':dz}, 
                    'moveRel_0': {'dx':0, 'dy':0, 'dz': inital_height},
...
for i in range(num_sampling-1):
    test_fnc(dict(soe=[f'orchestrator/wait_{i+1}',f'orchestrator/modify_{i+1}',f'lang:2/moveWaste_{i+1}', f'lang:2/RemoveDroplet_{i+1}', f'lang:2/moveSample_{i+1}',
                        f'lang:2/moveAbs_{i+1}',f'lang:2/moveRel_{i+1}', f'lang:2/moveDown_{i+1}',f'measure:2/schwefelFunction_{i+1}',f'analysis/dummy_{i+1}'], 
                params={f'wait_{i+1}':{'addresses':f'experiment_{i+1}:2/activeLearningParallel_{2*i+2}'},
                        f'modify_{i+1}':{'addresses':[f'experiment_{i+1}:2/activeLearningParallel_{2*i+2}/data/data/next_x_2',
                                                    f'experiment_{i+1}:2/activeLearningParallel_{2*i+2}/data/data/next_y_2',
                                                    f'experiment_{i+1}:2/activeLearningParallel_{2*i+2}/data/data/next_x_2',
                                                    f'experiment_{i+1}:2/activeLearningParallel_{2*i+2}/data/data/next_y_2'],
                                        'pointers':[f'schwefelFunction_{i+1}/x',f'schwefelFunction_{i+1}/y',f'moveAbs_{i+1}/dx', f'moveAbs_{i+1}/dy']},
                        f'moveWaste_{i+1}':{'x': 0, 'y':0, 'z':0},
                        f'RemoveDroplet_{i+1}': {'x':0, 'y':0, 'z':0},
...
test_fnc(dict(soe=['orchestrator/wait_0','ml/activeLearningParallel_0'],
                  params = {'wait_0':{'addresses':'experiment_0:0/dummy_0'},
                            'activeLearningParallel_0':{'num': 0,'query':query,'address':'experiment_0:0/dummy_0/data/data'}},meta={}),2)

for i in range(num_sampling):
    test_fnc(dict(soe=[f'orchestrator/wait_{2*i+1}',f'orchestrator/wait_{2*i+2}',f'ml/activeLearningParallel_{2*i+1}',f'ml/activeLearningParallel_{2*i+2}'], #,f'ml/activeLearning_{2*i+2}'
                  params = {f'wait_{2*i+1}':{'addresses':f'experiment_{i}:1/dummy_{i}'},
                            f'wait_{2*i+2}':{'addresses':f'experiment_{i+1}:0/dummy_{i+1}'},
                            f'activeLearningParallel_{2*i+1}':{'num': int(2*i+1),'query':query,'address':f'experiment_{i}:1/dummy_{i}/data/data'},
                            f'activeLearningParallel_{2*i+2}':{'num': int(2*i+2),'query':query,'address':f'experiment_{i+1}:0/dummy_{i+1}/data/data'}},meta={}),2)
