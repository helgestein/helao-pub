...
random.seed(1)
XY, C, I, Q = df[:,0:2], df[:,2:5], df[:,-2]*1e6, df[:,-1] # correct currents when real test is used (CP)
X, Y = -XY[:,0], XY[:,1]
x_query = np.array([[i, j] for i, j in zip(X, Y)])
query = json.dumps({'x_query': x_query.tolist(), 'c_query': C.tolist(), 'i_query': I.tolist(), 'q_query': Q.tolist()})
id0 = random.randint(0, len(X))
x0, y0, I0 = X[id0], Y[id0], Q[id0]/1000
...
test_fnc(dict(soe=['orchestrator/start', 'measure/coord_0', 'lang/getPos_0', f'autolab/measure_{0}', 'analysis/convert_0', 'ml/activeLearningGaussian_0'], 
            params={'start': {'collectionkey' : f'substrate_{substrate}'},
                    'coord_0': {'x': x0, 'y': y0},
                    'getPos_0': {},
                    f'measure_{0}': {'procedure': 'ocp_rs', 
                                            'setpointjson': json.dumps({'recordsignal': {'Duration': I0, 'Interval time in µs': 0.02}}),
                                            'plot': "tCV",
                                            'onoffafter': "off",
                                            'safepath': "C:/Users/LaborRatte23-2/Documents/GitHub/helao-dev_2/temp",
                                            'filename': 'substrate_{}_ocp_{}_{}.nox'.format(substrate, 0, 0),
                                            'parseinstructions':'recordsignal'},
                    'convert_0': {'query': query, 'address':json.dumps([f'experiment_0:0/coord_{0}/parameters/x',
                                                                        f'experiment_0:0/coord_{0}/parameters/y', #should be modified to moveSample x,y
                                                                        f'experiment_0:0/measure_{0}/data/data/recordsignal/Corrected time'])},
                    'activeLearningGaussian_0': {'name': 'sdc_4', 'num': int(0), 'query': query, 'address':f'experiment_0:0/convert_0/data'}},
            meta=dict()))

for i in range(n):
    test_fnc(dict(soe=[f'orchestrator/modify_{i}', f'measure/coord_{i+1}', f'lang/getPos_{i+1}', f'autolab/measure_{i+1}', f'analysis/convert_{i+1}', f'ml/activeLearningGaussian_{i+1}'],
                  params={f'modify_{i}': {'addresses':[f'experiment_{i}:0/activeLearningGaussian_{i}/data/next_x',
                                                       ...
                                                       f'experiment_{i}:0/activeLearningGaussian_{i}/data/next_i'],
                                            'pointers':[f'coord_{i+1}/x',f'coord_{i+1}/y',f'measure_{i+1}/setpointjson']},
                            f'coord_{i+1}': {'x': '?', 'y': '?'},
                            f'getPos_{i+1}': {},
                            f'measure_{i+1}': {'procedure': 'ocp_rs', 
                                            'setpointjson': '?',
                                            'plot': "tCV",
                                            'onoffafter': "off",
                                            'safepath': "C:/Users/LaborRatte23-2/Documents/GitHub/helao-dev_2/temp",
                                            'filename': 'substrate_{}_ocp_{}_{}.nox'.format(substrate, i+1, 0),
                                            'parseinstructions':'recordsignal'},
                            f'convert_{i+1}':{'query': query, 'address':json.dumps([f'experiment_{i+1}:0/coord_{i+1}/parameters/x',
                                                                                    f'experiment_{i+1}:0/coord_{i+1}/parameters/y',
                                                                                    f'experiment_{i+1}:0/measure_{i+1}/data/data/recordsignal/Corrected time'])},
