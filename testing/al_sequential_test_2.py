...
df = pd.read_csv(r'C:\Users\LaborRatte23-2\Documents\SDC functions\Python scripts\df_lim_98.csv').to_numpy()
random.seed(998)
...
test_fnc(dict(soe=['orchestrator/start', f'lang/moveWaste_{0}', f'hamilton/pumpL_{0}', f'hamilton/pumpR_{0}', f'lang/moveWaste_{1}', 
                    ...
                    f'hamilton/pumpL_{8}', f'lang/moveRel_{1}', f'hamilton/pumpL_{9}', f'lang/moveWaste_{5}', 
                    f'analysis/convert_{0}', f'ml/activeLearningGaussian_{0}'], 
            params={'start': {'collectionkey' : f'substrate_{substrate}'},
                    ...
                    f'moveDown_{0}': {'dz': 0.005, 'steps': 400, 'maxForce': 8000.0, 'threshold': 0.12},
                    f'getPos_{0}': {},
                    ...
                    f'measure_{1}':{'procedure': 'eis_fast',
                                    ...
                                    'onoffafter': 'off',
                                    'safepath': "C:/Users/LaborRatte23-2/Documents/GitHub/helao-dev_2/temp",
                                    'filename': 'substrate_{}_eis_ocp_{:02d}_{}.nox'.format(substrate, 0, 0),
                    ...
                    f'convert_{0}': {'query': query, 'address':json.dumps([f'experiment_0:0/moveSample_{0}/parameters/x',
                                                                           f'experiment_0:0/moveSample_{0}/parameters/y',
                                                                           f'experiment_0:0/measure_{3}/data/data/recordsignal/Corrected time'])},
                    f'activeLearningGaussian_{0}': {'name': 'sdc_4', 'num': int(0), 'query': query, 'address':f'experiment_0:0/convert_0/data'}},
            meta=dict()))
...
for i in range(n):
    test_fnc(dict(soe=[f'orchestrator/modify_{i}', f'lang/moveWaste_{6*(i+1)}', f'hamilton/pumpL_{10*(i+1)}', f'hamilton/pumpR_{i+1}', f'lang/moveWaste_{6*(i+1)+1}', 
                    ...
                    f'hamilton/pumpL_{10*(i+1)+5}', f'hamilton/pumpL_{10*(i+1)+6}', f'autolab/measure_{6*(i+1)}', f'autolab/measure_{6*(i+1)+1}', f'autolab/measure_{6*(i+1)+2}',
                    f'autolab/measure_{6*(i+1)+3}', f'autolab/measure_{6*(i+1)+4}', f'autolab/measure_{6*(i+1)+5}', f'hamilton/pumpL_{10*(i+1)+7}', f'lang/moveRel_{2*(i+1)}',
                    f'hamilton/pumpL_{10*(i+1)+8}', f'lang/moveRel_{2*(i+1)+1}', f'hamilton/pumpL_{10*(i+1)+9}', f'lang/moveWaste_{6*(i+1)+5}', 
                    f'analysis/convert_{i+1}', f'ml/activeLearningGaussian_{i+1}'],
            params={f'modify_{i}': {'addresses':[f'experiment_{i}:0/activeLearningGaussian_{i}/data/next_x',
                                    ...
                                    'pointers':[f'moveSample_{i+1}/x',f'moveSample_{i+1}/y',f'measure_{6*(i+1)+2}/setpointjson',f'measure_{6*(i+1)+3}/setpointjson']}, # correct ml_action for measure_cp
                    ...
                    f'moveDown_{i+1}': {'dz': 0.005, 'steps': 400, 'maxForce': 8000.0, 'threshold': 0.12},
                    f'getPos_{i+1}': {},
                    ...
                    f'measure_{6*(i+1)}': {'procedure': 'ocp_rs', 
                                        'setpointjson': json.dumps({'recordsignal': {'Duration': 60, 'Interval time in µs': 1}}),
                                        'plot': "tCV",
                                        'onoffafter': "off",
                                        'safepath': "C:/Users/LaborRatte23-2/Documents/GitHub/helao-dev_2/temp",
                                        ...
                                        'parseinstructions':'recordsignal'},
                    f'measure_{6*(i+1)+1}': {'procedure': 'eis_fast',
                                        'setpointjson': json.dumps({'FHSetSetpointPotential': {'Setpoint value': None}}),
                                        'plot': 'impedance',
                                        'onoffafter': 'off',
                                        'safepath': "C:/Users/LaborRatte23-2/Documents/GitHub/helao-dev_2/temp",
                                        'filename': 'substrate_{}_eis_ocp_{:02d}_{}.nox'.format(substrate, int(i+1), 0),
                                        'parseinstructions': ['FIAMeasPotentiostatic'],
                                        'substrate': substrate,
                                        'id': int(i+1),
                                        'experiment': 0},
                    f'measure_{6*(i+1)+2}': {'procedure': 'charge',
                                           'setpointjson': '?',
                                           'plot': 'tCV',
                                           'onoffafter': 'off',
                                           'safepath': "C:/Users/LaborRatte23-2/Documents/GitHub/helao-dev_2/temp",
                                           ...
                                           'parseinstructions': 'recordsignal'},
                    f'measure_{6*(i+1)+3}': {'procedure': 'discharge',
                                            'setpointjson': '?',
                                            'plot': 'tCV',
                                            'onoffafter': 'off',
                                            'safepath': "C:/Users/LaborRatte23-2/Documents/GitHub/helao-dev_2/temp",
                                            ...
                                            'parseinstructions': 'recordsignal'},
                    f'measure_{6*(i+1)+4}': {'procedure': 'ocp_rs', 
                                            'setpointjson': json.dumps({'recordsignal': {'Duration': 180, 'Interval time in µs': 1}}),
                                            'plot': "tCV",
                                            'onoffafter': "off",
                                            'safepath': "C:/Users/LaborRatte23-2/Documents/GitHub/helao-dev_2/temp",
                                            ...
                                            'parseinstructions':'recordsignal'},
                    f'measure_{6*(i+1)+5}': {'procedure': 'eis',
                                            'setpointjson': json.dumps({'FHSetSetpointPotential': {'Setpoint value': None}}),
                                            'plot': 'impedance',
                                            'onoffafter': 'off',
                                            'safepath': "C:/Users/LaborRatte23-2/Documents/GitHub/helao-dev_2/temp",
                    ...
                    f'convert_{i+1}':{'query': query, 'address':json.dumps([f'experiment_{i+1}:0/moveSample_{i+1}/parameters/x',
                                                                            f'experiment_{i+1}:0/moveSample_{i+1}/parameters/y',
                                                                            f'experiment_{i+1}:0/measure_{6*(i+1)+3}/data/data/recordsignal/Corrected time'])},
                    f'activeLearningGaussian_{i+1}': {'name': 'sdc_4', 'num': int(i+1), 'query': query, 'address':f'experiment_{i+1}:0/convert_{i+1}/data'}}, 
                  meta=dict()))
